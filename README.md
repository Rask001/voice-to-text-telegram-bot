...
- MVP-оплата Standard/Premium через Telegram Stars
...
app/
  ...
  payment_service.py  # Telegram Stars payload, payments, activation
...
Free заканчивается, если прошло больше 3 дней с момента регистрации или использовано 15 минут. После окончания trial бот предлагает оформить подписку.

## Telegram Stars

В `/profile` есть кнопка `⭐ Купить тариф`.
...
Цены и срок тарифа хранятся в `app/payment_service.py`:

- `STARS_TARIFFS`
- `TARIFF_DURATION_DAYS`

Оплата работает через Telegram Bot API invoices:

- currency: `XTR`
- provider token: пустая строка, потому что Telegram Stars не требует внешнего провайдера
- invoice отправляется только после выбора тарифа
- тариф активируется только после `successful_payment`
- `pre_checkout_query` только проверяет payload, тариф и сумму

Платежи сохраняются в SQLite таблицу `payments`. Повторный `successful_payment` с тем же `telegram_payment_charge_id` считается дублем и не продлевает тариф повторно.

Правила:

- Free может купить Standard или Premium;
- Standard может купить Premium;
- Premium может продлить Premium;
...
Под сообщением показывается inline-кнопка `⭐ Купить тариф`.
